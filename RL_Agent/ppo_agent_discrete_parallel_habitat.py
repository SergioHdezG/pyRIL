...
class Agent(PPOSuper):
    def __init__(self, actor_lr=1e-4, critic_lr=1e-3, batch_size=32, epsilon=1.0, epsilon_decay=0.9999, epsilon_min=0.1,
                 ...
                 img_input=False, is_habitat=False, state_size=None, n_threads=None, tensorboard_dir=None, net_architecture=None,
        ...
        self.agent_name = agent_globals.names["ppo_discrete_multithread_habitat"]
    ...
    def act(self, obs):
        ...
        action = self.action_selection_options(act_pred, self.n_actions, epsilon=self.epsilon, n_env=self.n_threads)
    ...
    def save_tensorboar_rl_histogram(self, histograms):
        if self.model.train_summary_writer is not None:
            with self.model.train_summary_writer.as_default():
                for [total_episodes, episodic_reward, epochs, success, spl, epsilon, global_steps] in histograms:
                    self.model.rl_sumaries([episodic_reward, epsilon, epochs, success, spl],
                                           ['Reward', 'Epsilon', 'Steps per episode', 'Success', 'SPL'],
                                           global_steps)