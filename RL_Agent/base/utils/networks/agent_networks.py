...
class HabitatPPOEmbodiedCLIPNet(HabitatPPONet):
    ...
    def fit(self, obs, next_obs, actions, rewards, done, epochs, batch_size, validation_split=0.,
        ...
        # TODO: [CARLOS] check if this split makes sense at all (specially the +1). Maybe using a ceiling instead of
        #   int in order to fit the rest of the observations.
        # TODO: [Sergio] Estandarizar para que batch_observations admita inputs variables. Es decir, consolidar
        #  batch_obs y batch_target en una sola variable que además pueda admitir batch_actions, batch_gps,
        #  batch_compass...
        batch_obs = np.array_split(obs[0], int(rewards.shape[0] / batch_size) + 1)
        batch_obs_target = np.array_split(obs[1], int(rewards.shape[0] / batch_size) + 1)
        batch_obs_actions = np.array_split(obs[2], int(rewards.shape[0] / batch_size) + 1)
        batch_rewards = np.array_split(rewards, int(rewards.shape[0] / batch_size) + 1)
        batch_mask = np.array_split(mask, int(rewards.shape[0] / batch_size) + 1)

        for b_o, b_t, b_a, b_r, b_m in zip(batch_obs, batch_obs_target, batch_obs_actions, batch_rewards, batch_mask):
            values = self.predict_values([b_o, b_t, b_a])
            ret, adv = self.calculate_advantages(values, b_m, b_r, gamma, lmbda)
            returns.extend(ret)
            advantages.extend(adv)
        ...
        for e in range(epochs):
            ...
            for batch, (batch_obs,
                ...
                advantages, \
                [act_comp_loss, critic_comp_loss, entropy_comp_loss] = self.train_step(
                    [tf.cast(batch_obs, tf.float32), tf.cast(batch_obs_target, tf.float32),
                     tf.cast(batch_obs_actions, tf.float32)],
                    tf.cast(batch_act_probs, tf.float32),
                    tf.cast(batch_actions, tf.float32),
                    tf.cast(batch_returns, tf.float32),
                    tf.cast(batch_advantages, tf.float32),
    ...
    def predict(self, x):
        # x is a list of n inputs positions list([np.array(x[1]), np.array(x[2]), ..., np.array(x[n])])
        y_ = self._predict(x)
        return y_.numpy()

    # TODO: [Sergio]: Standardize the inputs to _train_step(). We have two inputs (x[0]=rgb y x[1]=objectgoal) but in
    #   a generic problem we may have a different number of inputs. Pass this standarization to the super class.
    @tf.function(experimental_relax_shapes=False)
    def _predict(self, x):
        """ Predict the output sentence for a given input sentence
            Args:
            ...
            Returns:
                ...
                The output string array
            """
        out = self.actor_net(x, training=False)
        return out
    ...
    @tf.function(experimental_relax_shapes=False)
    def _predict_values(self, x):
        out = self.critic_net(x, training=False)
        return out
