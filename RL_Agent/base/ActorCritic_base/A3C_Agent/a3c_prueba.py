...
class ACNet(object):
    ...
    def _build_net(self, scope):  # neural network structure of the actor and critic
        with tf.variable_scope('actor'):
            ...
            mu = tf.keras.layers.Dense(N_A, activation='tanh', name='mu')(l_a)  # estimated action value
            sigma = tf.keras.layers.Dense(N_A, activation='softplus', name='sigma')(l_a)  # estimated variance
