...
class ACNet(object):
    def __init__(self, scope, sess, state_size, n_actions, stack=False, img_input=False, actor_lr=0.0001,
                 critic_lr=0.001, action_bound=None, globalAC=None, net_architecture=None):
        self.sess = sess
        ...
        if globalAC is None:  # get global network
            with tf.variable_scope(scope):
                if self.img_input or self.stack:
                    self.s = tf.placeholder(tf.float32, [None, *state_size], 'S')  # statetf.placeholder(tf.float32, [None, *state_size], 'S')  # state
                else:
    ...
    def _build_net(self, scope, net_architecture):  # neural network structure of the actor and critic
        ...
        with tf.variable_scope('actor'):
            if self.img_input:
                ...
                l_a = actor_model(self.s)
                # l_a = tf.keras.layers.Dense(256, activation='relu', name='la')(self.s)

            mu = tf.keras.layers.Dense(self.n_actions, activation='tanh', name='mu')(l_a)  # estimated action value
            sigma = tf.keras.layers.Dense(self.n_actions, activation='softplus', name='sigma')(l_a)  # estimated variance
        ...
        with tf.variable_scope('critic'):
            if self.img_input:
                ...
                # l_c = tf.keras.layers.Dense(128, activation='relu', name='lc')(l_c)
            else:
                critic_model = net_building.build_nn_net(net_architecture, self.state_size, critic=True)
                l_c = critic_model(self.s)
                # l_c = tf.keras.layers.Dense(128, activation='relu', name='lc')(self.s)
