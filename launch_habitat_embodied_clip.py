...
from RL_Agent.base.utils.networks.agent_networks import HabitatPPONet, HabitatPPOEmbodiedCLIPNet
...
# net = actor_model_emboided_clip_GRU_d5softmax((10, [(2048,), (6,)]))
# x1 = np.ones((2, 10, 2048))
# x2 = [np.array([0., 0., 0., 1., 0., 0.]) for i in range(2)]
# x2 = np.expand_dims(np.argmax(x2, axis=-1), axis=-1)
# x2 = np.repeat(x2, 10, axis=1)
# out = net([x1, x2])
...
def custom_model(input_shape):
    return HabitatPPOEmbodiedCLIPNet(input_shape,
                         actor_model,
                         critic_model,
                         n_stack,
