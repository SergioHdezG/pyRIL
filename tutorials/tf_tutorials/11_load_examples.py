...
def tf_custom_model(input_shape):
    agent_model = PPONet(actor_net=actor_custom_model(input_shape), critic_net=critic_custom_model(input_shape),
...
agent_cont = ppo_agent_continuous_parallel_tf.Agent(actor_lr=1e-5,
                                                 ...
                                                 tensorboard_dir='/home/shernandez/PycharmProjects/CAPOIRL-TF2/tutorials/tf_tutorials/tensorboard_logs/',
                                                 loads_saved_params=False  # Cuando se carga un agente guardado se usan los parámetros aquí especificados.
                                                                           # Por defecto loads_saved_params=True, en este caso se carga el estado en el que
                                                                           # se haya quedado el agente al guardarlo. Por ejemplo si se ha modificadpo el
                                                                           # valor de epsilon durante el entrenamiento y se quiere seguir desde donde se
                                                                           # quedó habrá que seleccionar loads_saved_params=True. Si por ejemplo se quiere
                                                                           # especificar un nuevo learning rate inicial habrá que definir los nuevos parámetros
                                                                           # e inicializar loads_saved_params=False.
                                                 )
...
# obs = deque(maxlen=agent_cont.n_stack)
# o = environment_cont.reset()
# for i in range(agent_cont.n_stack):
#     obs.append(o)
#
# while True:
#     environment_cont.render()
#     act = agent_cont.act(obs)
#     o, rew, done, info = environment_cont.step(act)
#     obs.append(o)
#     if done:
#         o = environment_cont.reset()
#         for i in range(agent_cont.n_stack):
...
# e = agent_cont.act(([0.5, 0.5, 0.5, 0.5, 1, 1, 0, 0],
#                    [0.5, 0.5, 0.5, 0.5, 1, 1, 0, 0],
#                    [0.5, 0.5, 0.5, 0.5, 1, 1, 0, 0],
#                    [0.5, 0.5, 0.5, 0.5, 1, 1, 0, 0]))
# agent_cont.actor.extract_variable_summaries = extract_variable_summaries

problem_cont.solve(100, render=False, max_step_epi=512, render_after=2090, skip_states=3)
...
# agent_saver.export_to_protobuf(agent_cont, 'agent_ppo')
